...
class Analysis:
    ...
    def satisfPercentage(self, df, cname, th):
        ...
        satisfPercentage = self.truncar(result, 4) * 100

        return satisfPercentage

    # Calcula el porcentaje de aparicion de cada temperatura
    def tempPercentages(self, df):

        total_temps = len(df["T"])

        results = list()
        
        # Calculamos las ocurrencias de cada una
        for temp in df["T"].unique():
            porcentaje = 100 * float(df.loc[df["T"] == temp, "T"].count()) / total_temps
            results.append([temp, porcentaje])

        return results

    # Calcula el porcentaje de aparicion de cada humedad
    def humPercentages(self, df):

        total_temps = len(df["H"])

        results = list()
        
        # Calculamos las ocurrencias de cada una
        for hum in df["H"].unique():
            porcentaje = 100 * float(df.loc[df["H"] == hum, "H"].count()) / total_temps
            results.append([hum, porcentaje])

        return results

    # Dibuja los porcentajes de ocurrencia
    def plotPercentages(self, array, title):

        fig = plt.figure()

        for row in array:
            plt.bar(row[0], float(row[1]))

        # plt.xticks(x, ("negativo", "positivo"))
        plt.xlabel(title)
        plt.ylabel("%")
        plt.title('Porcentaje de ocurrencia: ' + title)
        plt.show()
    ...
    # Dibuja en diagrama de barras las diferentes detecciones
    def drawBarDetections(self, data, titles):
        ...
        plt.title('Clasificacion de Opiniones')
        plt.show()
    ...
    # Calcula las visitas por horas (en conjunto)
    def entriesByHour(self, df):
        ...
        df_entries = df_copy.loc[df["Evento"] == "Entrada"].copy(deep=True)
        ...
        # Calculamos las ocurrencias
        counts = df_entries.groupby('Fecha')\
            .Evento.sum().to_frame().\
            rename(columns={"Fecha": "Horas", 'Evento': 'Entradas'})

        return counts

    # Dibuja el calculo de entradas por horas
    def plotByHour(self, df):

        df = df.reset_index().rename(columns={"Fecha": "Horas"})

        df.plot.bar(x="Horas", y="Entradas", colormap='Paired')
        plt.title("Visitas segun la hora")
        plt.show()

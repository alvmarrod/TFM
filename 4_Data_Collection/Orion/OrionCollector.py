...
# MODIFICACIONES ALVARO
#
# Ejecucion
# ./dataCollector.py --port 1028 --url /pushData --pretty-print -v
# Al recibir una peticion, lo almacenamos en MySQL
...
# Importamos nuestro conector de BBDD
...
def record():
    ...
    if verbose:
        ...
        if 'Evento' in request.data:
            print j['data'][0]['Fecha']['value'] + '\t' + \
                    'Evento: ' + j['data'][0]['Evento']['value'] + '\n'
                    
            datos = [j['data'][0]['Evento']['value'], j['data'][0]['Fecha']['value']]

            manager.write_data(datos)

        else:
            print j['data'][0]['Fecha']['value'] + '\t' + \
                "H: " + str(j['data'][0]['Humedad']['value']) + '\t' + \
                'T: ' + str(j['data'][0]['Temperatura']['value']) + '\n'
                
            datos = [j['data'][0]['Temperatura']['value'], \
                    j['data'][0]['Humedad']['value'], \
                    j['data'][0]['Fecha']['value']]
            ...
            try:
                ...
                # Puede que la conexion haya caducado.
                manager.close_db()
                #manager.db_connect("127.0.0.1", "alvmarrod", "alvmarrodtfm", "tfm")
                manager.db_connect("127.0.0.1", "root", "TFMUSAIT", "tfm")
                #dbLock.release()
...
if __name__ == '__main__':
    ...
    try:
        ...
        try:    
            manager.create_table('Museo')
        except Exception as error:
            print str(error)
            
        try:
            manager.create_table('Evento')
        except Exception as error:
            print str(error)
